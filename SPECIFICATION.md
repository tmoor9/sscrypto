...
Registry of pool, gauge, incentive and managed rewards factories. Contains a default list of factories so swaps via the router will always work.  Used within Voter to validate new gauge creation.  Reusing the same pool factory address with a new gauge or new incentive rewards factory is not permitted.
...
per epoch via Voter, with the votes earning NFT owners both incentives and fees
...
- Can bulk claim rewards (i.e. incentives + fees), incentives or fees.
...
Voting rewards are rewards that accrue to users that vote for a specific pool. They can be broken down into fees and incentives.

### FeesVotingReward

The fee voting reward derives from the fees relinquished by LP depositors
depositing their LP token in to the gauge. These fees are synchronized with incentives
...
Incentive voting rewards are externally deposited rewards of whitelisted tokens (see `Voter`) used to incentivize users to vote for a given pool.
